...
- V46: no NixOS release literal outside `flake.lock` (+ docs/spec prose) — scripts derive it: ISO file name from pinned nixpkgs `lib.trivial.release`, QEMU `init=`/`root=` from the built ISO's own grub.cfg
